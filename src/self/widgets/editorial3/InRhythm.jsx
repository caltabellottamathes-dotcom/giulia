...
/** InRhythm — STATE / LIVE · 9:16. Full-bleed ghosted figure, grote type
 *  "IN RHYTHM" + capacity-ring die vult + tellend percentage. */
const R = 30, C = 2 * Math.PI * R;

export default function InRhythm() {
  const [val, setVal] = useState(0);
  useEffect(() => { const t = setTimeout(() => setVal(78), 300); return () => clearTimeout(t); }, []);
  ...
  return (
    <WidgetShell size="1x1" radius="large" interactive onClick={() => {}} className="min-h-0" style={{ aspectRatio: "9 / 16", "--tile-accent": PLUM }}>
      <div className="relative h-full w-full overflow-hidden">
        <motion.img src={PHOTOS3.ghostFigure} alt="" className="absolute inset-0 h-full w-full object-cover" initial={{ scale: 1.14, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 1.6, ease: [0.16, 1, 0.3, 1] }} />
        ...
        <div className="absolute inset-0 p-4 flex flex-col justify-between text-ivory">
          <motion.p className="text-[9px] uppercase tracking-[0.28em] font-bold opacity-70" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25, duration: 0.5 }}>How I'm Doing</motion.p>
          <div className="flex flex-col gap-3">
            <motion.h2 className="text-[36px] leading-[0.9] font-display font-bold tracking-[-0.04em]" initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35, duration: 0.7, ease: [0.16, 1, 0.3, 1] }}>IN<br />RHYTHM</motion.h2>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <span className="text-[8px] uppercase tracking-[0.2em] opacity-65">capacity</span>
              </div>
            </div>
          </div>
        </div>
