...
/** TwentyThreeDays — PEOPLE / SOCIAL · 1:1. Vase-petal foto + giant "23" +
 *  "dagen gezien" + pulserende ringen rond de oranje accent (recentie). */
export default function TwentyThreeDays() {
  return (
    <WidgetShell size="1x1" radius="large" interactive onClick={() => {}} className="min-h-0" style={{ aspectRatio: "1 / 1", "--tile-accent": PLUM }}>
      <div className="relative h-full w-full overflow-hidden">
        <motion.img src={PHOTOS3.vasePetal} alt="" className="absolute inset-0 h-full w-full object-cover" initial={{ scale: 1.14, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 1.6, ease: [0.16, 1, 0.3, 1] }} />
        ...
        <div className="absolute inset-0 p-4 flex flex-col justify-end text-ivory">
          <motion.p className="text-[9px] uppercase tracking-[0.28em] font-bold opacity-70 mb-1" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25, duration: 0.5 }}>Social · Jill</motion.p>
          <div className="flex items-end gap-2">
            <motion.span initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35, duration: 0.7, ease: [0.16, 1, 0.3, 1] }}>
              <CountUp value={23} className="text-[68px] leading-[0.8] font-display font-bold tabular-nums tracking-[-0.05em] block" />
            </motion.span>
            <motion.span className="text-[13px] font-display font-semibold opacity-70 mb-2 leading-tight" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6, duration: 0.5 }}>dagen<br />gezien</motion.span>
          </div>
        </div>
      </div>
    </WidgetShell>
