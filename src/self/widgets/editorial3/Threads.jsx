...
/** Threads — NETWORK / MATERIAL · 1:1. Glove-thread foto + radiale draden die
 *  naar buiten tekenen + centrale "verbonden %" teller + "WOVEN" type. */
export default function Threads() {
  const [val, setVal] = useState(0);
  useEffect(() => { const t = setTimeout(() => setVal(72), 300); return () => clearTimeout(t); }, []);
  ...
  return (
    <WidgetShell size="1x1" radius="large" interactive onClick={() => {}} className="min-h-0" style={{ aspectRatio: "1 / 1", "--tile-accent": PLUM }}>
      <div className="relative h-full w-full overflow-hidden">
        <motion.img src={PHOTOS3.gloveThread} alt="" className="absolute inset-0 h-full w-full object-cover" initial={{ scale: 1.14, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 1.6, ease: [0.16, 1, 0.3, 1] }} />
        ...
        <div className="absolute left-1/2 top-[55%] -translate-x-1/2 -translate-y-1/2 flex flex-col items-center">
          ...
          <span className="text-[7px] uppercase tracking-[0.2em] text-ivory/70 mt-0.5">verbonden %</span>
        </div>
        <div className="absolute inset-0 p-4 flex flex-col justify-between text-ivory pointer-events-none">
          <motion.p className="text-[9px] uppercase tracking-[0.28em] font-bold opacity-70" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25, duration: 0.5 }}>Network · woven</motion.p>
          <motion.h2 className="text-[28px] leading-[0.9] font-display font-bold tracking-[-0.04em] self-end" initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4, duration: 0.7, ease: [0.16, 1, 0.3, 1] }}>WOVEN</motion.h2>
        </div>
      </div>
    </WidgetShell>
  );
