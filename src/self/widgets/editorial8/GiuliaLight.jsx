...
/** GiuliaLight — grote foto + zacht glas: Giulia's levende kern (state + heartbeat). · 1:1 */
export default function GiuliaLight() {
  const { openModule } = usePanel();
  const [idx, setIdx] = useState(0);
  useEffect(() => { const id = setInterval(() => setIdx((i) => (i + 1) % STATES.length), 1800); return () => clearInterval(id); }, []);
  return (
    <PhotoCardLight photo={PHOTO} onClick={() => openModule("chat")} aspectRatio="1 / 1"
      top={<><p className="text-[10px] uppercase tracking-[0.24em] font-bold opacity-70">Giulia</p><h3 className="text-[36px] leading-[0.84] font-display font-semibold tracking-[-0.04em] mt-0.5">GIULIA</h3></>}>
      ...
      <div className="h-[28px] overflow-hidden flex items-center mt-2">
        <AnimatePresence mode="wait" initial={false}>
          <motion.span key={idx} className="text-[20px] font-display font-semibold tracking-[-0.02em]" style={{ color: BURG }} initial={{ y: 18, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -18, opacity: 0 }} transition={{ duration: 0.4, ease: "easeOut" }}>{STATES[idx]}</motion.span>
        </AnimatePresence>
      </div>
    </PhotoCardLight>
  );
