...
/** ApprovalsLight — grote foto + zacht glas met wacht-teller + zachte rijen. · 2:3 */
export default function ApprovalsLight() {
  const { openModule } = usePanel();
  const learnTick = useLearningSync();
  const { data } = useEntityList("Approval", { filter: { status: "pending" }, sort: "-created_date", limit: 20, externalTick: learnTick });
  const list = data || [];
  return (
    <PhotoCardLight photo={PHOTO} onClick={() => openModule("approvals")} aspectRatio="2 / 3"
      top={<><p className="text-[10px] uppercase tracking-[0.24em] font-bold opacity-70">Waiting on You.</p><h3 className="text-[32px] leading-[0.84] font-display font-semibold tracking-[-0.04em] mt-0.5">WACHTEN</h3></>}>
      <div className="flex items-end gap-3 mb-3">
        <span className="text-[44px] font-display font-semibold tabular-nums leading-none" style={{ color: BURG }}>{list.length}</span>
