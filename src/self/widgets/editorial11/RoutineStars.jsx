...
/** RoutineStars — ROUTINES / STREAK · 3:4. Numbers-neck foto + "14 / 30" grote
 *  cijfers + constellation die verbindt (14 routine-dagen deze maand). */
const PTS = [
  { x: 22, y: 26, n: 3 }, { x: 48, y: 18, n: 8 }, { x: 74, y: 30, n: 11 },
  { x: 34, y: 50, n: 9 }, { x: 66, y: 56, n: 17 }, { x: 50, y: 72, n: 20 },
  { x: 26, y: 80, n: 24 }, { x: 76, y: 84, n: 30 },
];
const LINKS = [[0, 1], [1, 2], [1, 3], [3, 4], [3, 5], [4, 5], [5, 6], [5, 7]];

export default function RoutineStars() {
  return (
    <WidgetShell size="1x1" radius="large" interactive onClick={() => {}} className="min-h-0" style={{ aspectRatio: "3 / 4", "--tile-accent": PLUM }}>
      <div className="relative h-full w-full overflow-hidden">
        <motion.img src={PHOTOS11.numbersNeck} alt="" className="absolute inset-0 h-full w-full object-cover" initial={{ scale: 1.14, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 1.6, ease: [0.16, 1, 0.3, 1] }} />
        ...
        <div className="absolute inset-0 p-4 flex flex-col justify-between text-ivory pointer-events-none">
          <motion.p className="text-[9px] uppercase tracking-[0.28em] font-bold opacity-70" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25, duration: 0.5 }}>Routines · 14 van 30</motion.p>
