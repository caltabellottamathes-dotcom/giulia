...
export default function Insights() {
  ...
  const top = insights.slice(0, 3).map((ins, i) => ({
    n: pad2(i + 1),
    title: ins.title || "Observatie",
    desc: `${ins.category || "Observatie"}${ins.source ? ` · ${ins.source}` : ""}`,
  }));

  return (
    <GiuliaAdminShell
      pageKey="insights"
      eyebrow="GIULIA → INSIGHTS"
      title="What I've Noticed"
      related={[{ label: "Memory", to: "/memory" }, { label: "Wants to know", to: "/wants-to-know" }, { label: "Chat", to: "/chat" }]}
      hero={IMAGES.wWhatIveNoticed}
      card={{
        eyebrow: "What I've noticed | insights_",
        title1: "Patterns,", title2: "signals, ideas.",
        metaLine: `${newCount} nieuw · ${insights.length} observaties`,
        heading1: "Worth", heading2: "a look",
        itemsLabel: `${pad2(top.length)}_new_noticed_`,
        items: top,
      }}
    >
      <div className="space-y-5">
        {/* Insights tabs — Observaties / Self / Journal / Routines */}
        <div className="flex flex-wrap gap-2">
          ...
          <button onClick={() => setShowNew(!showNew)} className="ml-auto h-8 px-4 rounded-full bg-charcoal text-ivory text-xs font-bold hover:bg-charcoal/90 transition inline-flex items-center gap-1.5">
            <Sparkles className="h-3.5 w-3.5" /> Noteer
          </button>
        </div>
        ...
        {tab === "obs" && (
          <>
            ...
            <div className="flex gap-3 overflow-x-auto pb-1">
              {[
                { label: "Nieuw", count: newCount, accent: "bg-olive text-ivory" },
