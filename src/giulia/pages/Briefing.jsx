...
/**
 * Briefing — fashion-editorial catch-up. Laadt bij iedere keer verse content
 * op basis van alle nieuwe data. Echte gestapelde kaarten: veeg een kaart weg
 * en hij is weg (vliegt uit), de volgende schuift op. Achtergrond = geblurd,
 * geen overlay.
 */
export default function Briefing() {
  ...
  const buildItems = useCallback((rawItems, answered) => {
    ...
    picks.forEach((q, i) => {
      ...
      result.splice(pos, 0, { ...q, type: "question", id: `q-${q.key}`, status: "new", suggested_action: "Vertel het Giulia", priority: "relevant" });
    });
    return result;
  }, []);
  ...
  const load = useCallback(async () => {
    ...
    try {
      // Verse briefing-items op basis van alle nieuwe data (best-effort; lokaal
      // blijft de briefing sowieso vers door directe entity-lees).
      await base44.functions.invoke("compileBriefing", {}).catch(() => {});
  ...
  // Naar outro als de stapel leegraakt.
  useEffect(() => { if (phase === "stack" && deck.length === 0) setPhase("outro"); }, [phase, deck.length]);
  ...
  const swipe = useCallback((dir) => {
    ...
    if (top) updateStatus(top.id, dir > 0 ? "actioned" : "later");
  ...
  const openAction = (item) => {
    if (!item) return;
    if (item.type === "question") { navigate("/chat"); return; }
    updateStatus(item.id, "actioned");
    const params = item.action_params && Object.keys(item.action_params).length ? "?" + new URLSearchParams(item.action_params).toString() : "";
  ...
  const onDone = useCallback(async (taskId) => {
    ...
    try {
      ...
      base44.entities.Activity.create({
        action: "task_completed", description: (cur?.title || "Taak").slice(0, 120),
        source: "briefing", timestamp: new Date().toISOString(),
      }).catch(() => null);
    } catch {}
    ...
    return (
      <div className="fixed inset-0 z-[120] bg-warm-white text-charcoal flex animate-fade-in overflow-hidden">
        ...
        <div className="absolute right-0 top-0 h-full w-1/2 hidden sm:block">
          <div className="absolute top-6 right-6 inline-flex items-center gap-1.5 rounded-full bg-ivory/75 backdrop-blur-xl border border-white/60 px-3 py-1.5 text-[10px] uppercase tracking-[0.22em] font-bold text-charcoal">Giulia Briefing</div>
        </div>
        <div className="relative w-full sm:w-1/2 flex flex-col justify-end sm:justify-center px-8 sm:px-14 lg:px-20 pt-10 sm:pt-0 pb-14 sm:pb-0">
          ...
          <h1 className="text-[clamp(2.5rem,7vw,5rem)] font-display font-bold leading-[0.95] tracking-[-0.035em] mb-5 text-balance">Ik heb de boel in de gaten gehouden.</h1>
          ...
          <div className="flex flex-col sm:flex-row gap-3 max-w-md">
            <button onClick={() => setPhase("stack")} className="h-12 px-7 rounded-full bg-charcoal text-ivory font-bold text-sm hover:bg-charcoal/90 transition inline-flex items-center justify-center gap-2">Start briefing <ArrowRight className="h-4 w-4" /></button>
            <button onClick={askGiulia} className="h-12 px-7 rounded-full bg-ivory/70 backdrop-blur-xl border border-charcoal/15 text-charcoal font-semibold text-sm hover:bg-ivory transition inline-flex items-center justify-center gap-2"><Sparkles className="h-4 w-4" /> Ask Giulia</button>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="fixed inset-0 z-[120] bg-warm-white text-charcoal flex flex-col justify-center px-6 animate-fade-in overflow-hidden">
        ...
        <div className="relative max-w-xl mx-auto w-full text-center">
          <div className="h-16 w-16 rounded-full bg-olive/15 border border-olive/30 flex items-center justify-center mx-auto mb-6"><Sparkles className="h-7 w-7 text-olive" /></div>
          <h1 className="text-[clamp(2.5rem,6vw,4rem)] font-display font-bold leading-[0.98] tracking-[-0.03em] mb-3">{data?.outro?.head || "Je bent weer bij."}</h1>
          <p className="text-lg text-charcoal/65 mb-2">{data?.outro?.subline}</p>
          {!data?.outro?.next && <div className="mb-8" />}
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <button onClick={() => navigate("/")} className="h-12 px-7 rounded-full bg-charcoal text-ivory font-bold text-sm hover:bg-charcoal/90 transition">Naar dashboard</button>
            <button onClick={askGiulia} className="h-12 px-7 rounded-full bg-ivory/70 backdrop-blur-xl border border-charcoal/15 text-charcoal font-semibold text-sm hover:bg-ivory transition inline-flex items-center justify-center gap-2"><Sparkles className="h-4 w-4" /> Ask Giulia</button>
          </div>
        </div>
      </div>
  ...
  const doneCount = items.length - deck.length;

  return (
    <div className="fixed inset-0 z-[120] bg-warm-white text-charcoal flex flex-col animate-fade-in overflow-hidden">
      {/* Geen overlay — alleen geblurde editorial achtergrond */}
      <img src={IMAGES.salvoFeetPebbles} alt="" className="absolute inset-0 h-full w-full object-cover scale-110 blur-2xl pointer-events-none" />

      {/* Top bar */}
      <div className="relative flex items-center justify-between px-5 py-4">
        <button onClick={() => navigate("/")} className="inline-flex items-center gap-1.5 rounded-full bg-ivory/70 backdrop-blur-xl border border-charcoal/10 px-3.5 py-1.5 text-[12px] font-semibold text-charcoal/80 hover:bg-ivory transition"><X className="h-3.5 w-3.5" /> Sluiten</button>
        <div className="flex items-center gap-1.5">
          {items.map((_, i) => (
            <span key={i} className={`h-1 rounded-full transition-all ${i === doneCount ? "w-6 bg-charcoal" : i < doneCount ? "w-1.5 bg-charcoal/40" : "w-1.5 bg-charcoal/20"}`} />
          ))}
        </div>
        <button onClick={askGiulia} className="inline-flex items-center gap-1.5 rounded-full bg-olive text-ivory px-3.5 py-1.5 text-[12px] font-bold hover:bg-olive/90 transition"><Sparkles className="h-3.5 w-3.5" /> Ask Giulia</button>
      </div>

      {/* Card stage — echte stapel */}
        ...
        <div className="relative w-[min(92vw,440px)] h-[min(78vh,660px)] lg:w-[min(90vw,440px)] lg:h-[min(70vh,620px)]">
          <AnimatePresence custom={exitDir}>
            {[...visible].reverse().map((item, ri) => {
              const depth = visible.length - 1 - ri; // 0 = bovenste
              const isTop = depth === 0;
              return (
                <motion.div
                  key={item.id}
                  className="absolute inset-0 cursor-grab active:cursor-grabbing"
                  style={{ zIndex: 10 - depth, rotate: isTop ? rotate : 0 }}
                  initial={{ scale: 0.9, y: 24, opacity: 0 }}
                  animate={{ scale: 1 - depth * 0.05, y: depth * 14, opacity: depth === 0 ? 1 : 0.6 }}
                  exit={(dir) => ({ x: dir > 0 ? 1400 : -1400, y: 0, opacity: 0, scale: 0.9, transition: { duration: 0.34, ease: [0.22, 1, 0.36, 1] } })}
                  transition={{ type: "spring", stiffness: 300, damping: 30 }}
                  drag={isTop ? "x" : false}
                  dragConstraints={{ left: 0, right: 0 }}
                  dragElastic={0.7}
                  dragSnapToOrigin
                  onDrag={(_, info) => { if (isTop) xDrag.set(info.offset.x); }}
                  onDragEnd={(_, info) => {
                    if (info.offset.x > 120 || info.velocity.x > 600) { xDrag.set(0); swipe(1); }
                    else if (info.offset.x < -120 || info.velocity.x < -600) { xDrag.set(0); swipe(-1); }
                    else { xDrag.set(0); }
                  }}
                >
                  {isTop && (
                    <>
                      <motion.div style={{ opacity: likeOp }} className="absolute top-10 left-7 z-30 pointer-events-none -rotate-12"><span className="text-4xl sm:text-5xl font-display font-black text-olive border-[3px] border-olive rounded-xl px-3 py-0.5 tracking-tight">ACTIE</span></motion.div>
                      <motion.div style={{ opacity: nopeOp }} className="absolute top-10 right-7 z-30 pointer-events-none rotate-12"><span className="text-4xl sm:text-5xl font-display font-black text-destructive border-[3px] border-destructive rounded-xl px-3 py-0.5 tracking-tight">LATER</span></motion.div>
                    </>
                  )}
                  <BriefingCard
                    item={item}
                    onAct={() => openAction(item)}
                    onAnswer={onAnswer}
                    onDone={onDone}
                    onExplain={() => explain(item)}
                    expanded={expanded}
                    onToggleExpand={() => setExpanded((v) => !v)}
                    photoIndex={doneCount}
                  />
                </motion.div>
              );
            })}
      ...
      <div className="relative px-5 pb-6 pt-2">
        <div className="max-w-md mx-auto flex items-center justify-between gap-3">
          <button onClick={() => swipe(-1)} className="flex-1 h-11 rounded-2xl bg-ivory/70 backdrop-blur-xl border border-charcoal/12 text-charcoal/75 text-[13px] font-semibold hover:bg-ivory transition inline-flex items-center justify-center gap-2"><ChevronLeft className="h-4 w-4" /> Later</button>
          <span className="text-[11px] uppercase tracking-wider text-charcoal/40 font-bold px-2 hidden sm:block">veeg</span>
          <button onClick={() => swipe(1)} className="flex-1 h-11 rounded-2xl bg-charcoal text-ivory text-[13px] font-bold hover:bg-charcoal/90 transition inline-flex items-center justify-center gap-2">Actie <ChevronRight className="h-4 w-4" /></button>
        </div>
        <p className="text-center text-[11px] text-charcoal/45 mt-3 font-medium">{doneCount + 1} van {items.length} · ← later · → actie</p>
      </div>
    </div>
  );
}