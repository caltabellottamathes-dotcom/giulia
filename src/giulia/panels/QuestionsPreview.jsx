...
/** QuestionsPreview (body-only) — rendert alleen de Body-inhoud voor het
 *  universele ModulePanel. De contextrij + knoppen worden via `onFooter`
 *  aan het paneel geleverd, dat ze in de vaste footer zet (zonder achtergrond).
 *  `onOpen` opent de volledige ruimte. */
export default function QuestionsPreview({ onOpen, onFooter }) {
  const [items, setItems] = useState([]);
  const [loading, setLoading] = useState(true);
  const [busy, setBusy] = useState(false);

  const load = async () => {
    try { const list = await base44.entities.GiuliaQuestion.filter({ status: "open" }, "-created_date", 12); setItems(list || []); }
    catch { setItems([]); }
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[220px_1fr] gap-6 h-full overflow-hidden">
      <div className="flex flex-col gap-5 overflow-auto pr-1">
        <div className="flex flex-col items-center">
          <AnimatedRing pct={items.length ? Math.min(100, items.length * 10) : 0} size={150} color={MID} label={String(items.length)} sub="OPEN" />
        </div>
        <div>
      ...
      <div className="flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-auto pr-1 space-y-2">
          <AnimatePresence>
            {loading ? (
              <p className="text-storm/40 text-sm">Laden…</p>
            ) : items.length === 0 ? (
              ...
              <motion.div
                ...
                exit={{ opacity: 0 }}
                onClick={onOpen}
                className="rounded-2xl border border-marble/25 bg-marble/8 px-4 py-3.5 cursor-pointer hover:bg-marble/12 transition-colors"
              >
                <div className="flex items-center gap-2 mb-1.5">
                  <span className="text-[9px] uppercase tracking-wider font-semibold px-2 py-0.5 rounded-full" style={{ background: `${(PRIORITY[q.priority] || PRIORITY.useful).c}22`, color: (PRIORITY[q.priority] || PRIORITY.useful).c }}>
                    {(PRIORITY[q.priority] || PRIORITY.useful).l}
                  </span>
                ...
                {q.options && q.options.length ? (
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    {q.options.map(o => (
                      <button key={o} onClick={(e) => { e.stopPropagation(); answer(q, o); }} className="px-2.5 py-1 rounded-full border border-marble/20 bg-marble/5 text-[11px] text-storm/80 hover:bg-marble/15 transition">{o}</button>
                    ))}
                  </div>
                ) : (
