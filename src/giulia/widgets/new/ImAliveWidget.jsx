 ...
 *  de widget → startGiulia (start Giulia + alle agenten). Styling als de
 *  andere GIULIA widgets: ivoor tekst, olijf EKG, urgent accent. */
        ...
        <div className="flex flex-col h-full p-3" style={{ color: IVORY }}>
          ...
          <div className="flex-1 relative min-h-0">
            <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="absolute inset-0 w-full h-full">
              <line x1="0" y1="50" x2="100" y2="50" stroke={DEEP} strokeOpacity="0.22" strokeWidth="0.4" />
              {active ? (
                <motion.path d={PATH} fill="none" stroke={DEEP} strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"
                  initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 1.4, ease: "easeInOut", repeat: Infinity, repeatType: "loop" }} />
              ) : (
                <path d="M 0 50 L 100 50" fill="none" stroke={DEEP} strokeOpacity="0.4" strokeWidth="1" />
              )}
            </svg>
          ...
          <div className="flex items-center justify-between mt-1 gap-0.5">
            {STATES.map((s, i) => (
              <span key={s} className="text-[6.5px] uppercase tracking-[0.08em] font-bold" style={{ opacity: active && i === idx ? 1 : 0.32, color: active && i === idx ? LIGHT : IVORY }}>{s}</span>
            ))}
          </div>
          {!active && (
