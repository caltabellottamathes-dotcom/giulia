        ...
        <div className="ml-auto flex gap-1">
          {["create", "link", "skip"].map((a) => (
            <button key={a} onClick={() => onChange({ action: a })}
              className={`text-[10px] uppercase font-semibold px-2.5 py-1 rounded-full transition inline-flex items-center ${action === a ? (a === "create" ? "bg-olive text-white" : a === "link" ? "bg-sand-deep text-white" : "bg-muted-foreground/30 text-muted-foreground") : "glass-1 text-muted-foreground"}`}>
              {a === "create" ? <Check className="w-3 h-3 mr-0.5" /> : a === "link" ? <Link2 className="w-3 h-3 mr-0.5" /> : <X className="w-3 h-3 mr-0.5" />}
              {a === "create" ? "Maak" : a === "link" ? "Koppel" : "Skip"}
            </button>
          ))}
        </div>
      </div>
