  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-3">
        ...
        <Stat label="Categorie" value={items[0]?.category || "—"} />
      </div>
        ...
        <div className="space-y-2">
          {items.map((a) => (
            <div key={a.id} className="rounded-2xl px-4 py-3 glass-1">
              <button onClick={onOpen} className="w-full text-left group flex items-start gap-2">
                ...
                <ArrowUpRight className="h-3.5 w-3.5 text-foreground/30 group-hover:text-foreground/60 transition shrink-0 mt-0.5" />
              </button>
