...
export default function EmailWidget() {
  ...
  return (
    <WidgetShell size="2x2" radius="large" interactive onClick={() => openModule("email")} className="min-h-[160px]">
        ...
        {loading ? (
          ...
          <>
            <div className="flex items-end gap-3">
              <CountUp value={hero} className="text-[52px] leading-[0.85] font-display font-semibold tracking-[-0.04em] text-current" />
            ...
            <div className="mt-3 flex gap-1">
              {Array.from({ length: 14 }).map((_, i) => {
                ...
                return (
                  <span key={i} className="h-6 flex-1 rounded-[3px] transition-all duration-500" style={filled ? { background: isUrgent ? "var(--tile-accent)" : "currentColor", opacity: isUrgent ? 1 : 0.85 } : { background: "currentColor", opacity: 0.1 }} />
                );
              })}
            </div>
      ...
      {!loading && (
        <BrandPhoto
          ...
          className="h-16 w-full -mt-8 rounded-t-[24px] relative z-10 shadow-[0_-12px_28px_-12px_rgba(0,0,0,0.28)]"
