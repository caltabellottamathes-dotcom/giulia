...
export default function WhatsAppWidget() {
  ...
  return (
    <WidgetShell size="2x2" radius="medium" interactive onClick={() => openModule("whatsapp")} className="min-h-[160px]">
      <div className="flex flex-col h-full">
        <BrandPhoto src={IMAGES.stilettoHead} className="h-16 -mb-8 rounded-b-[24px] shadow-[0_14px_28px_-12px_rgba(0,0,0,0.3)] relative z-10" overlay="bg-gradient-to-t from-charcoal/85 to-charcoal/30">
            ...
            <div>
              ...
              {unreadTotal > 0 && (
                <div className="flex items-end gap-2">
                  <CountUp value={unreadTotal} className="text-xl font-display font-semibold tracking-[-0.03em] leading-none text-ivory" />
          ...
          {loading ? (
            ...
            <>
              ...
              <form onSubmit={(e) => { e.preventDefault(); send(e); }} onClick={(e) => e.stopPropagation()} className="mt-auto pt-3 flex items-center gap-2">
                <input value={reply} onChange={(e) => setReply(e.target.value)} placeholder={`Antwoord aan ${nameOf(top.contact_id)}…`} className="flex-1 min-w-0 bg-current/5 border border-current/15 rounded-full px-3 py-2 text-sm text-current placeholder:text-current/40 focus:outline-none focus:border-current/40" />
                <button type="submit" className="h-9 w-9 rounded-full flex items-center justify-center font-semibold transition hover:-translate-y-0.5 active:scale-95 shrink-0" style={{ background: "var(--tile-accent)", color: "var(--tile-on-accent)" }}>→</button>
              </form>
            </>
          ) : (
