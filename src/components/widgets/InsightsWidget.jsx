...
/**
 * InsightsWidget — trends as a bespoke confidence sparkline (route-line of
 * recent insight certainty). Hero is the count of fresh insights; a sculpted
 * button lets Giulia run proactive research.
 */
export default function InsightsWidget() {
  ...
  const pts = recent.length
    ? recent.map((ins, i) => `${(recent.length === 1 ? 0 : (i / (recent.length - 1)) * 100)},${30 - (ins.confidence || 0.5) * 30}`).join(" ")
    : "";
    ...
    try {
      const res = await base44.integrations.Core.InvokeLLM({
        ...
        response_json_schema: {
          type: "object",
          properties: { title: { type: "string" }, content: { type: "string" }, category: { type: "string" }, confidence: { type: "number" } },
        },
      });
