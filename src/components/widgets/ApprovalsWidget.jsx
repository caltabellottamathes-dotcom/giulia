...
export default function ApprovalsWidget() {
  ...
  return (
    <WidgetShell size="2x1" radius="soft" interactive onClick={() => openModule("approvals")} className="min-h-[150px]">
        ...
        <div className="relative h-20 shrink-0 overflow-hidden">
          ...
          <div className="absolute inset-0 p-5 flex flex-col justify-between">
            ...
            <div className="flex items-end gap-2">
              <CountUp value={approvals.length} className="text-3xl font-display font-semibold tracking-[-0.03em] leading-none text-ivory" />
              <p className="text-[10px] uppercase tracking-[0.2em] text-ivory/70 mb-1.5">wachten op jou</p>
            </div>
          </div>
        </div>
        <div className="flex-1 -mt-8 rounded-t-[24px] glass-3 p-3 relative z-10 shadow-[0_-12px_28px_-12px_rgba(0,0,0,0.35)] text-ivory flex flex-col">
          {loading ? (
            ...
            <>
              ...
              <div className="mt-auto pt-3 flex gap-2">
                <button onClick={(e) => decide(e, top, "approve")} className="flex-1 h-9 rounded-2xl font-semibold text-sm transition hover:-translate-y-0.5 active:scale-95" style={{ background: "var(--tile-accent)", color: "var(--tile-on-accent)" }}>Goedkeuren</button>
                <button onClick={(e) => decide(e, top, "reject")} className="flex-1 h-9 rounded-2xl font-semibold text-sm border border-ivory/20 text-ivory transition hover:bg-ivory/10">Afwijzen</button>
              </div>
            </>
          ) : (
