...
export default function PeopleWidget() {
  ...
  return (
    <WidgetShell size="2x1" radius="medium" interactive onClick={() => openModule("people")} className="min-h-[150px]">
            ...
            <>
              <div className="flex items-center">
                <div className="flex -space-x-3">
                  {cluster.map((c, i) => (
                    <span key={c.id} className="h-9 w-9 rounded-full border-2 border-ivory/20 flex items-center justify-center text-sm font-semibold" style={{ background: "var(--tile-accent)", color: "var(--tile-on-accent)", zIndex: 10 - i }}>
              ...
              <div className="mt-auto pt-3 flex items-center justify-between gap-2">
                ...
                <a href={top?.phone ? `tel:${top.phone}` : undefined} onClick={(e) => e.stopPropagation()} className={cn("h-9 px-4 rounded-2xl font-semibold text-sm flex items-center transition hover:-translate-y-0.5 active:scale-95", !top?.phone && "pointer-events-none opacity-40")} style={{ background: "var(--tile-accent)", color: "var(--tile-on-accent)" }}>Bel</a>
              </div>
            </>
          ) : (
