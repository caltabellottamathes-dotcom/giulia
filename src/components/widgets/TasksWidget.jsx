...
/**
 * TasksWidget — one question: how much is left, and how late?
 * Hero is a bespoke completion ring (done / total) with the open count
 * centered. An accent pulse pill flags overdue items.
 */
export default function TasksWidget() {
  ...
  const complete = async (e, task) => {
    e.stopPropagation();
    try { await base44.entities.Task.update(task.id, { status: "completed" }); reload(); } catch {}
  };
      ...
      <div className="p-5 flex flex-col h-full">
        ...
        {loading ? (
          ...
          <div className="flex-1 flex items-center gap-5">
            <Ring value={done.length} max={total || 1} size={124} stroke={12}>
            ...
            <div className="flex-1 min-w-0">
              ...
              {top ? (
                <>
                  ...
                  <button onClick={(e) => complete(e, top)} className="mt-3 rounded-full px-4 py-2 text-[12px] font-semibold transition hover:-translate-y-0.5 active:scale-95" style={{ background: "var(--tile-accent)", color: "var(--tile-on-accent)" }}>
                    Voltooi
                  </button>
                </>
              ) : (
