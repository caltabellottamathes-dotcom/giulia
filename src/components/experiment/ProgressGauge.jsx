...
/**
 * ProgressGauge — widget-family glass card with a branded photo header and an
 * olive progress ring. Same glass-2 material, app fonts and palette as the
 * dashboard widgets. The whole card is clickable (opens the module); the pill
 * and action button stop propagation.
 */
export default function ProgressGauge({ image, label, count, percent = 0, title, subtitle, pillLabel, onPillClick, actionIcon: ActionIcon, onAction, onClick }) {
  ...
  return (
    <div
      onClick={onClick}
      className="glass-2 rounded-3xl overflow-hidden text-foreground w-full max-w-[360px] mx-auto cursor-pointer hover:-translate-y-0.5 transition-transform"
    >
      ...
      <div className="p-6">
        ...
        <div className="flex items-center justify-between gap-2 mt-5">
          <div className="flex gap-2">
            {ActionIcon && (
              <button
                ...
                className="h-9 w-9 rounded-full border border-foreground/15 bg-foreground/5 flex items-center justify-center hover:bg-foreground/10 transition"
                aria-label="Actie"
              >
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 rounded-full bg-foreground text-background px-4 py-2 text-xs font-medium hover:bg-foreground/90 transition"
          >
