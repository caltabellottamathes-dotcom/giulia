...
export default function PaginaOntwerp() {
  ...
  return (
    <div className="fixed inset-x-0 top-14 bottom-0 overflow-hidden z-[1]">
      {/* Hero photo — links, iets lager */}
      <motion.div initial={{ x: "-118%" }} animate={{ x: 0 }} transition={{ duration: 0.7, ease: EASE }}
        className="hidden lg:block absolute left-0 top-[20%] bottom-[4%] w-[34%] overflow-hidden rounded-r-[24px] z-[5]">
      ...
      {/* Glazen paneel — iets lager */}
      <motion.div initial={{ x: "118%" }} animate={{ x: 0 }} transition={{ duration: 0.7, ease: EASE }}
        className="absolute right-0 top-[2%] bottom-[4%] w-full lg:w-[76%] glass-2 rounded-l-[32px] rounded-r-none shadow-[0_64px_150px_-34px_rgba(0,0,0,0.55), -20px_0_70px_-34px_rgba(0,0,0,0.32)] flex z-[15]">
        ...
        {/* Witte kaart — links ~42% editorial (geen scroll, past exact), rechts leeg */}
        <motion.div initial={{ opacity: 0, y: 28, scale: 0.985 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.6, ease: EASE, delay: 0.32 }}
          ...
          <div className="absolute inset-0 rounded-l-[20px] rounded-r-none bg-white flex overflow-hidden shadow-[-28px_24px_64px_-22px_rgba(0,0,0,0.42)]">
            {/* Editorial — linker ~42%, past exact in de kaarthoogte */}
            <div className="w-[42%] h-full flex flex-col overflow-hidden border-r" style={{ borderColor: GREY }}>
              <div className="flex-1 flex flex-col min-h-0 px-6 lg:px-8 pt-7 pb-6">
                ...
                {/* Verdeel de witruimte — past exact, geen scroll */}
                <div className="flex-1 min-h-4" />

                {/* Sectie-scheidingslijn + marker */}
                ...
                {/* Footer — onderaan */}
                <div className="pt-6 border-t" style={{ borderColor: GREY }}>
