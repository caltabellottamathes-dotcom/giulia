...
const BounceBalls = ({ colors = ["#000"], count = 3, className = "" }) => (
  <span className={`inline-flex items-end gap-[3px] align-baseline ${className}`} aria-hidden>
    {Array.from({ length: count }).map((_, i) => (
      <span key={i} className="ontwerp-dot-bounce inline-block rounded-full bg-current" style={{ color: colors[i % colors.length], width: "clamp(7px, 0.55vw, 10px)", height: "clamp(7px, 0.55vw, 10px)", animationDelay: `${i * 0.18}s` }} />
    ))}
  </span>
);
...
export default function OntwerpWhiteCard({ tab }) {
  ...
  const s2Second = s2rest.length ? s2rest.join("|").trim() : "";
  return (
    <motion.div
      initial={{ x: "100%" }}
      animate={{ x: 0 }}
      ...
      <div className="w-[42%] h-full flex flex-col overflow-hidden border-r" style={{ borderColor: GREY }}>
        <div className="flex-1 flex flex-col min-h-0 px-6 lg:px-8 pt-7 pb-6">
          <div className="flex items-center justify-between">
            <p className="font-mono text-[10px] tracking-[0.18em]" style={{ color: BLUE }}><span className="font-bold">{eyeA.trim()}</span>{eyeB}</p>
            <span className="font-mono text-[10px] tracking-[0.18em]" style={{ color: BLUE }}>N°1</span>
          </div>

          <h2 className="font-display font-bold tracking-[-0.035em] leading-[0.92] mt-6" style={{ color: BLACK, fontSize: "clamp(34px, 3vw, 54px)", textShadow: "0 0 18px rgba(177,191,199,0.45), 0 0 36px rgba(177,191,199,0.22)" }}>
          ...
          <h3 className="font-display font-bold tracking-[-0.025em] leading-[0.98] mb-5" style={{ color: BLACK, fontSize: "clamp(24px, 1.9vw, 38px)" }}>
            {c.heading1}<br />{h2Clean}<BounceBalls colors={NUM_COLORS} className="ml-[7px]" />
          </h3>
          ...
          <div className="mt-5 flex items-start justify-between gap-4">
            <div className="flex gap-3 min-w-0">
              <p className="font-mono text-[10px] tracking-[0.18em] font-bold whitespace-nowrap shrink-0" style={{ color: BLUE }}>{s2a.trim()}{s2Second ? " |" : ""}</p>
              <div className="flex-1 min-w-0">
                {s2Second && <p className="font-mono text-[10px] tracking-[0.18em]" style={{ color: BLUE }}>{s2Second}</p>}
                <p className="font-mono text-[10px] tracking-[0.18em] mt-1" style={{ color: BLUE }}>{c.itemsLabel}</p>
                <div className="space-y-3 mt-3">
                  {c.items.map((it, idx) => (
                    <button key={it.n} onClick={() => navigate(`/life/personal-admin?tab=${tab}`)} className="flex items-center gap-2 text-left w-full hover:opacity-70 transition">
                      <BounceBalls colors={[NUM_COLORS[idx % 3]]} count={idx + 1} className="mr-1 shrink-0" />
            ...
            <span className="font-mono text-[10px] tracking-[0.18em] shrink-0" style={{ color: BLUE }}>N°2</span>
          </div>
