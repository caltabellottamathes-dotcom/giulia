...
import { closeCircle, socialPulse, whatsappThreads } from "@/lib/domainUtils";
...
/** SocialPlannerPreview — sociale tijd inplannen: vrije momenten, afspraken
 *  deze week, open uitnodigingen en mensen die je wilde zien. */
export default function SocialPlannerPreview({ onOpen }) {
  ...
  const load = async () => {
    try {
      const [c, e, p, w] = await Promise.all([
      ...
      setContacts(c || []); setEvents(e || []); setPlans(p || []); setWhatsapps(w || []);
  ...
  const freeSlots = useMemo(() => {
    ...
    for (let i = 0; i < 10 && slots.length < 3; i++) {
      const d = new Date(now.getTime() + i * 86400000); d.setHours(19, 0, 0, 0);
  ...
  const weekEvents = useMemo(() => { const s = startOfWeek().getTime(), e = s + 7 * 86400000; return (events || []).filter((ev) => { const t = new Date(ev.start).getTime(); return t >= s && t < e; }); }, [events]);
  const pulse = useMemo(() => socialPulse(closeCircle(contacts)), [contacts]);
  const peopleWant = pulse.filter((p) => p.overdue);
  const opportunities = freeSlots.map((s, i) => ({ date: s, hours: 3, person: peopleWant[i] }));
  const openPlans = (plans || []).filter((p) => p.status === "planned" || p.status === "tentative");
  const threads = useMemo(() => whatsappThreads(whatsapps, contacts, 3), [whatsapps, contacts]);
  ...
  const createPlan = async () => {
    ...
    try {
      const start = form.slot.toISOString(); const end = new Date(form.slot.getTime() + 2 * 3600000).toISOString();
      ...
      setForm({ contactId: "", activity: "", slot: null }); await load();
    } catch { /* ignore */ }
  };
  ...
  return (
    <PreviewShell index="24" section="SOCIAL PLANNER" statement={status} kicker={`${freeSlots.length} VRIJE MOMENTEN`} accent={LIGHT}
      context={[
        ...
        { label: "DEZE WEEK", text: `${weekPlans.length} plannen · ${weekEvents.length} afspraken.` },
        { label: "KANSEN", text: opportunities.length ? `${opportunities.length} sociale kansen gevonden.` : "Geen extra kansen." },
      ]}
      actions={[{ label: "Pulse", to: "/life/social?view=socialpulse" }, { label: "Persoonlijke Tijd", primary: true, to: "/life/social?view=socialtime" }]}>
      <div className="grid grid-cols-1 lg:grid-cols-[200px_1fr] gap-5 h-full overflow-hidden">
        <div className="flex flex-col gap-4 overflow-auto pr-1">
          ...
          <div>
            <p className="text-storm/50 text-[10px] tracking-[0.25em] mb-2">VRIJE SLOTS</p>
            <div className="space-y-1.5">
              {freeSlots.map((s) => (
                <button key={s.toISOString()} onClick={() => setForm((f) => ({ ...f, slot: s }))} className={`w-full text-left rounded-lg px-3 py-2 text-xs border transition ${form.slot?.toISOString() === s.toISOString() ? "bg-sand text-storm border-sand" : "border-marble/30 bg-marble/5 text-storm/70 hover:bg-marble/10"}`}>{fmtSlot(s)}</button>
              ))}
        ...
        <div className="flex flex-col overflow-hidden">
          <p className="text-storm/50 text-[10px] tracking-[0.25em] mb-2">SOCIALE KANSEN</p>
          <div className="flex-1 overflow-auto pr-1 space-y-2">
            {loading ? <p className="text-storm/40 text-sm">Laden…</p> : (
              <>
                {opportunities.length ? opportunities.map((o, i) => (
                  <div key={i} className="rounded-2xl border border-marble/20 bg-marble/5 p-3">
                    ...
                    {o.person && <button onClick={() => setForm((f) => ({ ...f, slot: o.date, contactId: o.person.contact.id }))} className="mt-2 inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-semibold" style={{ color: PLUM, background: LIGHT }}><Plus className="w-3.5 h-3.5" />Plan iets</button>}
                  </div>
                )) : <p className="text-storm/40 text-sm">Geen kansen — week is vol.</p>}

                <p className="text-storm/50 text-[10px] tracking-[0.25em] mb-2 mt-3">AFSPRAKEN DEZE WEEK</p>
                {weekEvents.length ? weekEvents.map((ev) => (
                  <div key={ev.id} onClick={onOpen} className="flex items-center gap-3 rounded-xl border border-marble/20 bg-marble/5 px-3 py-2.5 cursor-pointer hover:bg-marble/10 transition">
                    <CalendarHeart className="w-3.5 h-3.5 text-storm/50 shrink-0" />
                    <span className="text-sm text-storm flex-1 truncate">{ev.title}</span>
                    <span className="text-[10px] text-storm/40 tabular-nums">{fmtDay(ev.start)}</span>
                  </div>
                )) : <p className="text-storm/45 text-sm italic">Geen afspraken deze week.</p>}

                <p className="text-storm/50 text-[10px] tracking-[0.25em] mb-2 mt-3">OPEN PLANNEN</p>
                {openPlans.length ? openPlans.map((p) => (
                  <div key={p.id} className="rounded-xl border border-marble/20 bg-marble/5 p-3">
                    <div className="flex items-center justify-between gap-2">
                      <div className="min-w-0"><p className="text-sm font-semibold text-storm truncate">{p.activity}</p><p className="text-[11px] text-storm/50 flex items-center gap-1"><Clock className="w-3 h-3" /> {new Date(p.suggested_date).toLocaleString("nl-NL", { weekday: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })} · {(p.contact_ids || []).map(contactName).join(", ")}</p></div>
                ...
                )) : <p className="text-storm/45 text-sm italic">Geen open plannen.</p>}
              </>
            )}
          </div>
        </div>
