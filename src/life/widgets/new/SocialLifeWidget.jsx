...
/** SocialLifeWidget — P·4:5 · foto-strip boven + GlassShell beneden met een
 *  grote Close Circle (SocialOrbit #16): centraal portret met cirkelende
 *  contacten; radius = recentie, urgente relaties in Urgent. */
export default function SocialLifeWidget() {
  ...
  return (
    <div className="relative w-full aspect-[4/5] rounded-[28px] overflow-hidden" onClick={() => openModule("social")} style={{ cursor: "pointer", color: IVORY }}>
      {/* foto + PhotoStrip boven */}
      <img src={PHOTO} alt="What Social Life" className="absolute inset-0 w-full h-full object-cover" draggable={false} />
      <div className="absolute top-0 inset-x-0 h-[42%] flex flex-col p-4 pb-3" style={{ background: "linear-gradient(to bottom, rgba(0,0,0,0.42), rgba(0,0,0,0.04))", textShadow: "0 1px 6px rgba(0,0,0,0.45)" }}>
        ...
        <div className="flex items-end gap-2.5 mt-auto">
          <span className="text-[44px] leading-[0.8] font-display font-semibold tabular-nums" style={{ color: overdueCount ? URGENT : IVORY }}>{interactions}</span>
          <p className="text-[9px] uppercase tracking-[0.18em] opacity-70 mb-1 leading-tight">interacties<br />30 dagen</p>
        </div>
      </div>

      {/* GlassShell beneden met Close Circle diagram */}
      <div className="absolute bottom-0 inset-x-0 h-[58%] rounded-t-[28px] flex flex-col p-4 overflow-hidden"
        ...
        <div className="flex items-center justify-between mb-2">
          <span className="text-[9px] uppercase tracking-[0.22em] font-bold">Close Circle</span>
          <span className="text-[8px] uppercase tracking-[0.14em] opacity-55">{activePlans} plannen</span>
        </div>
        <div className="flex-1 relative flex items-center justify-center min-h-0">
          <div className="relative w-full h-full max-w-[340px] max-h-[340px] aspect-square">
            <svg viewBox="0 0 100 100" className="absolute inset-0 w-full h-full">
              {[24, 34, 44].map((rr, i) => (
                <circle key={i} cx="50" cy="50" r={rr} fill="none" stroke={IVORY} strokeWidth="0.35" opacity="0.16" strokeDasharray="2 2.5" />
              ))}
              {orbit.map((c, i) => {
                ...
                return (
                  <g key={i}>
                    <line x1="50" y1="50" x2={x} y2={y} stroke={c.overdue ? URGENT : LIGHT} strokeWidth="0.5" opacity={c.overdue ? 0.8 : 0.4} />
                    <circle cx={x} cy={y} r="2.4" fill={c.overdue ? URGENT : LIGHT} />
                  </g>
                );
              })}
            </svg>
