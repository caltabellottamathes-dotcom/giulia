...
import { SAND, PLUM } from "./lifeColors";
import { Loader2, Sparkles, Check, ArrowRight } from "lucide-react";

/** TAB 5 — GIULIA. 1) Food Profile → 2) Plan nieuwe week → 3) bijsturen → 4) Accept. */
export default function GiuliaTab({ reload, goToTab }) {
  ...
  const planWeek = async () => {
    ...
    try { const res = await base44.functions.invoke("planFoodWeek", {}); setResult(res); reload(); }
    catch (e) { setError(String(e?.message || e)); } finally { setPlanning(false); }
  };
  ...
  return (
    <div className="space-y-5">
      ...
      <GlassPanel level={2} className="p-6">
        ...
        <button onClick={planWeek} disabled={planning} className="rounded-full px-6 py-3 text-sm font-bold text-ivory inline-flex items-center gap-2 disabled:opacity-50 hover:scale-[1.02] transition-transform" style={{ background: PLUM }}>
      ...
      {result && (
        <GlassPanel level={2} className="p-6 animate-fade-up">
          ...
          <h3 className="text-3xl font-display font-semibold tracking-tight mt-1">Je week is klaar</h3>
          <div className="flex flex-wrap gap-8 mt-4">
            <Stat label="Budget" value={`${fmtEuro(result.total_cost)} / ${fmtEuro(result.budget)}`} color={PLUM} />
            <Stat label="Maaltijden" value={result.meals_count} color={PLUM} />
            <Stat label="Quick" value={result.quick_meals} color={SAND} />
            <Stat label="Aanbiedingen" value={result.promotions} color={PLUM} />
          </div>
          <p className="text-sm text-muted-foreground mt-4">Pas individuele gerechten aan in de <b>Planning</b>-tab — verplaatsen of verwijderen.</p>
          <div className="flex flex-wrap gap-2 mt-5">
            ...
            <button onClick={() => goToTab && goToTab("volgende")} className="rounded-full px-5 py-2.5 text-sm font-bold text-ivory inline-flex items-center gap-1.5" style={{ background: PLUM }}><Check className="h-4 w-4" /> Accept week</button>
          </div>
        </GlassPanel>
      )}
    </div>
...
function Stat({ label, value, color }) {
  return (
    <div>
      <p className="text-xl font-display font-semibold tabular-nums" style={{ color }}>{value}</p>
