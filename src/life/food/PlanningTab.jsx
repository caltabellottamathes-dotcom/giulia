...
export default function PlanningTab({ week, meals, reload }) {
  ...
  if (!week) {
    return <GlassPanel level={1} className="p-10 text-center text-muted-foreground">Plan eerst een week in de Giulia-tab.</GlassPanel>;
  }
    ...
    <div className="space-y-4">
      <GlassPanel level={2} className="p-5">
        <div className="flex items-center justify-between mb-1">
          ...
          <span className="text-xs text-muted-foreground">{fmtEuro(week.total_cost)} / {fmtEuro(week.budget)}</span>
        </div>
        ...
        <div className="min-w-[640px] grid grid-cols-[84px_repeat(4,1fr)] gap-2">
          ...
          {MEAL_ORDER.map((mt) => (
            <div key={mt} className="flex items-center gap-2 px-2 pb-1">
              <MealTypeIcon type={mt} className="h-4 w-4 text-muted-foreground" />
          ...
          {days.map((d) => (
            <React.Fragment key={d.date}>
              <div className="flex flex-col justify-center px-1 py-2">
                <span className="text-xs font-bold uppercase tracking-wider text-foreground/70">{DAY_LABELS[d.dayKey]}</span>
              ...
              {MEAL_ORDER.map((mt) => {
                ...
                return (
                  <button
                    key={mt}
                    onClick={m ? () => setSelected(m) : undefined}
                    className={`text-left rounded-xl p-2.5 border transition min-h-[56px] ${m ? "bg-card border-border hover:border-olive hover:bg-muted/60" : "border-dashed border-border/60"}`}
                  >
                    {m ? (
                      <>
                        <p className="text-sm font-display font-semibold leading-tight">{m.recipe_name}</p>
                        <p className="text-[10px] text-muted-foreground mt-1">{m.total_time ? `${m.total_time}m · ` : ""}{fmtEuro(m.cost)}</p>
                      </>
                    ) : (
      ...
      {selected && (
        <RecipeView
          ...
          onMove={() => { setSelected(null); reload(); }}
          onEaten={() => { setSelected(null); reload(); }}
        />
      )}
    </div>
  );
