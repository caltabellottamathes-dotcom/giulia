...
/**
 * FoodProfileCard — toont het Food Profile waarop een week gebaseerd is.
 * Read-only tenzij `editable`; in bewerk-modus kun je budget, personen,
 * maaltijden en voorkeuren aanpassen en opslaan naar de FoodProfile-entity.
 */
export default function FoodProfileCard({ profile, editable, onSaved }) {
  ...
  const toggleMeal = (mt) => {
    setForm((f) => ({ ...f, meals: f.meals.includes(mt) ? f.meals.filter((x) => x !== mt) : [...f.meals, mt] }));
  };

  const inputCls = "w-full rounded-xl border border-border bg-background px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-ring/30";
      ...
      <div className="rounded-[24px] bg-card border border-border p-6 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={save} disabled={saving} className="h-8 w-8 rounded-full bg-charcoal text-ivory flex items-center justify-center disabled:opacity-50">{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Check className="h-4 w-4" />}</button>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2 mt-1.5">
            {MEAL_ORDER.map((mt) => (
              <button key={mt} onClick={() => toggleMeal(mt)} className={cn("rounded-full px-3 py-1.5 text-xs font-medium border transition", form.meals.includes(mt) ? "bg-charcoal text-ivory border-charcoal" : "border-border text-muted-foreground")}>
                {MEAL_LABELS[mt]}
              </button>
            ))}
          </div>
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          <label className="space-y-1"><span className="text-xs text-muted-foreground">Voorkeuren (komma)</span>
            <input value={form.likes} onChange={(e) => setForm((f) => ({ ...f, likes: e.target.value }))} className={inputCls} placeholder="bv. pasta, kip" />
          </label>
          <label className="space-y-1"><span className="text-xs text-muted-foreground">Niet gewenst (komma)</span>
            <input value={form.dislikes} onChange={(e) => setForm((f) => ({ ...f, dislikes: e.target.value }))} className={inputCls} placeholder="bv. paddenstoelen" />
          </label>
          <label className="space-y-1"><span className="text-xs text-muted-foreground">Favorieten</span>
            <input value={form.favourites} onChange={(e) => setForm((f) => ({ ...f, favourites: e.target.value }))} className={inputCls} />
          </label>
          <label className="space-y-1"><span className="text-xs text-muted-foreground">Keukens</span>
            <input value={form.cuisines} onChange={(e) => setForm((f) => ({ ...f, cuisines: e.target.value }))} className={inputCls} placeholder="bv. italiaans, aziatisch" />
          </label>
        </div>
      </div>
    );
  ...
  return (
    <div className="rounded-[24px] bg-card border border-border p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        {editable && (
          <button onClick={() => setEditing(true)} className="inline-flex items-center gap-1.5 rounded-full bg-muted px-3 py-1.5 text-xs font-medium text-muted-foreground hover:text-foreground transition">
            <Pencil className="h-3 w-3" /> Bewerk
          </button>
        )}
      </div>
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <Field label="Budget" value={profile?.weekly_budget ? `${fmtEuro(profile.weekly_budget)}/wk` : "—"} />
        <Field label="Personen" value={profile?.people ?? "—"} />
