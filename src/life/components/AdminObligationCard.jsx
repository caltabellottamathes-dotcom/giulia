...
export default function AdminObligationCard({ item, action = "OPEN", onAction, tone = "light", extra }) {
  if (!item) return null;
  ...
  return (
    <div className="rounded-2xl p-4" style={{ background: dark ? "rgba(120,122,128,0.12)" : "hsl(var(--card))", boxShadow: dark ? "none" : "0 14px 34px -26px rgba(0,0,0,0.28)" }}>
      <div className="flex items-center justify-between gap-2">
        ...
        <span className="text-[10px] uppercase tracking-wide font-semibold shrink-0" style={{ color: c }}>{labelOf(s)}</span>
      </div>
