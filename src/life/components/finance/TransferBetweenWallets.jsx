...
export default function TransferBetweenWallets() {
  ...
  return (
    <div className="w-full rounded-[18px] bg-[#f5f5f4] p-4 flex flex-wrap items-center gap-3" style={{ boxShadow: "-16px 16px 40px -16px rgba(0,0,0,0.30)" }}>
      ...
      <select value={fromId} onChange={(e) => setFromId(e.target.value)} className="rounded-xl glass-1 px-3 py-2 text-sm outline-none min-w-[150px]">
        <option value="">Van wallet…</option>
        {active.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
      </select>
      <ArrowRight className="w-4 h-4 text-muted-foreground shrink-0" />
      <select value={toId} onChange={(e) => setToId(e.target.value)} className="rounded-xl glass-1 px-3 py-2 text-sm outline-none min-w-[150px]">
        <option value="">Naar wallet…</option>
        {active.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
      </select>
      <div className="flex items-center gap-2 ml-auto">
        <div className="flex items-center rounded-xl glass-1 pl-3 pr-1 py-1">
