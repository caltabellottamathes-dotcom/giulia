    ...
    <div className="w-full h-64">
      <ResponsiveContainer width="100%" height="100%">
        <AreaChart data={data} margin={{ top: 8, right: 8, left: -16, bottom: 0 }}>
          <defs>
            {keys.map((k, i) => (
              <linearGradient key={k} id={`g-${k}`} x1="0" y1="0" x2="0" y2="1">
                ...
                <stop offset="100%" stopColor={PALETTE[i % PALETTE.length]} stopOpacity={0.25} />
              </linearGradient>
            ))}
          </defs>
          ...
          />
          {keys.map((k, i) => (
            <Area key={k} type="monotone" dataKey={k} stackId="1" stroke={PALETTE[i % PALETTE.length]} strokeWidth={1.5} fill={`url(#g-${k})`} />
          ))}
        </AreaChart>
      </ResponsiveContainer>
    </div>
