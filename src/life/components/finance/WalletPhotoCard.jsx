...
export default function WalletPhotoCard({ wallet, expenses, photoUrl }) {
  ...
  return (
    <div className="relative w-full h-full rounded-[14px] overflow-hidden" style={{ background: "#f5f5f4", border: "1px solid rgba(0,0,0,0.06)", boxShadow: "-16px 16px 40px -16px rgba(0,0,0,0.30)", color: INK }}>
      ...
      <button type="button" onClick={openWallet} className="absolute top-0 left-0 right-0 h-1/2 z-0 p-3 flex flex-col text-left cursor-pointer hover:bg-foreground/[0.03] transition-colors">
        <div className="flex items-center justify-between">
          <p className="text-[9px] uppercase tracking-[0.2em] font-semibold" style={{ color: MUTED }}>Saldo</p>
          <ArrowUpRight className="w-3.5 h-3.5" style={{ color: MUTED }} />
        </div>
        <p className="text-[22px] leading-none font-display font-semibold tabular-nums mt-1">{fmtEuro(balance)}</p>
        <div className="mt-auto w-full space-y-2">
          <div>
            <div className="flex items-end justify-between mb-1">
              <p className="text-[8px] uppercase tracking-[0.16em] font-semibold" style={{ color: MUTED }}>{goal1 > 0 ? "Doel 1 · Dekking" : "Doel 1 · geen lasten"}</p>
              <p className="text-[13px] leading-none font-display font-semibold tabular-nums">{fill1}%</p>
            </div>
          ...
          <div>
            <div className="flex items-end justify-between mb-1">
              <p className="text-[8px] uppercase tracking-[0.16em] font-semibold" style={{ color: MUTED }}>Doel 2 · Buffer</p>
              <p className="text-[13px] leading-none font-display font-semibold tabular-nums">{bufferProg}%</p>
            </div>
            <div className="h-1.5 rounded-full bg-foreground/10 overflow-hidden">
      ...
      <button type="button" onClick={openWallet} className="absolute bottom-0 left-0 right-0 h-1/2 z-0 p-3 overflow-hidden flex flex-col text-left cursor-pointer hover:bg-foreground/[0.03] transition-colors">
        <p className="text-[9px] uppercase tracking-[0.2em] font-semibold shrink-0" style={{ color: MUTED }}>Vaste lasten · {potExpenses.length}</p>
        <div className="mt-1.5 space-y-1 overflow-y-auto no-scrollbar flex-1 min-h-0">
          ...
          {potExpenses.slice(0, 6).map((e) => (
            <div key={e.id} className="flex items-center justify-between gap-2 rounded-lg bg-foreground/[0.04] px-2 py-1">
              ...
              <span className="text-[10px] font-display font-semibold tabular-nums shrink-0">{fmtEuro(e.expected_amount ?? e.amount)}</span>
            </div>
          ))}
        </div>
      </button>
      ...
      >
        ...
        <div className="absolute inset-0 p-3 flex flex-col text-white" style={{ textShadow: "0 1px 6px rgba(0,0,0,0.45)" }}>
          ...
          <h3 className="text-[15px] leading-tight font-display font-semibold tracking-[-0.02em] mt-auto">{wallet.name}</h3>
