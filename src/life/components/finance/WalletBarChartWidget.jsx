...
/**
 * WalletBarChartWidget — glazen widget-shell (glas-2) met brede BarChart links +
 * fotokaart rechts (zelfde foto als linker PhotoCard) die naar links schuift bij
 * selectie. 100% hoogte = Huur (Wonen). Elke bar = huidig saldo van een
 * portefeuille, geschaald t.o.v. de huur, gecapped op 100% (als er meer in zit
 * dan huur → even hoog als huur). Kleuren = portefeuille-kleuren.
 */
export default function WalletBarChartWidget() {
  ...
  // 100% referentie = de Huur (Wonen). Skaleer alle bars t.o.v. de huur, capped op 100%.
  const huurAmt = useMemo(() => {
    const h = (expenses || []).find((e) => String(e.title || "").toLowerCase().replace(/\.$/, "") === "huur");
    const wonen = (portfolios || []).find((p) => p.name.toLowerCase() === "wonen");
    return (
      (h ? h.expected_amount ?? h.amount : 0) ||
      (wonen ? wonen.current_balance || wonen.monthly_reservation_actual : 0) ||
      Math.max(1, ...wallets.map((w) => w.balance))
    );
  }, [expenses, portfolios, wallets]);
    ...
    <div className="relative w-full h-full rounded-[18px] overflow-hidden glass-2">
      ...
      <AnimatePresence>
        {!selected && (
          ...
          >
            ...
            <div className="flex-1 flex items-end gap-[clamp(2px,0.3vw,5px)] mt-3 min-h-0">
              {wallets.length === 0 && (
                <p className="text-[11px] text-foreground/40 self-center w-full text-center">No wallets yet.</p>
              )}
              {wallets.map((w) => {
                const h = Math.min(100, (w.balance / huurAmt) * 100);
                return (
                  <button
                    key={w.id}
                    onClick={() => setSelectedId(w.id)}
                    className="relative flex-1 h-full hover:opacity-90 transition min-w-0 group"
                    title={w.name}
                  >
                    {/* color bar — breed, volledige kolombreedte */}
                    <div
                      className="absolute bottom-0 left-0 right-0 rounded-t-md transition-all duration-500"
                      style={{ height: `${h}%`, background: w.color, boxShadow: "0 6px 16px -8px rgba(0,0,0,0.35)" }}
                    />
                    <span className="absolute -top-0.5 left-0 right-0 text-[8px] font-mono text-center text-foreground/55 truncate">{w.name.split(" ")[0]}</span>
                  </button>
                );
              })}
            </div>
      ...
      >
        ...
        {selected ? (
          <div className="absolute inset-0 p-4 flex flex-col text-ivory" style={{ textShadow: "0 1px 6px rgba(0,0,0,0.5)" }}>
            ...
            <p className="text-[10px] uppercase tracking-[0.16em] mt-1 opacity-80">reservering {fmt(selected.reservation)}/mnd</p>
      ...
      <AnimatePresence>
        {selected && (
          ...
          >
            <WalletDetail wallet={selected} huurAmt={huurAmt} />
          </motion.div>
        )}
      </AnimatePresence>
    </div>
...
function WalletDetail({ wallet, huurAmt }) {
  const pct = huurAmt > 0 ? Math.min(100, (wallet.balance / huurAmt) * 100) : 0;
  return (
    <div className="h-full p-4 flex flex-col text-foreground">
      ...
      <div className="mt-5 space-y-2.5">
        <div className="flex items-center justify-between text-[10px]">
          <span className="uppercase tracking-[0.14em] opacity-60">vs Huur</span>
          <span className="font-medium">{Math.round(pct)}%</span>
        </div>
        <div className="h-1.5 w-full rounded-full bg-foreground/10 overflow-hidden">
          <div className="h-full rounded-full" style={{ width: `${pct}%`, background: wallet.color }} />
        </div>
        <p className="text-[11px] opacity-80">Reservering <span className="font-semibold">{fmt(wallet.reservation)}/mnd</span></p>
      </div>
