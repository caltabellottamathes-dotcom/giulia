    ...
    <div className="relative w-full h-[380px] rounded-[28px] overflow-hidden" style={{ background: "#f5f5f4", border: "1px solid rgba(0,0,0,0.06)", boxShadow: "-16px 16px 40px -16px rgba(0,0,0,0.30)" }}>
      ...
      <div className="absolute inset-y-0 right-0 w-[42%] rounded-[28px] overflow-hidden z-20" style={{ boxShadow: "-16px 0 36px -20px rgba(0,0,0,0.5), inset 0 1px 0 rgba(255,255,255,0.16)" }}>
        ...
        <motion.div className="absolute inset-0" animate={{ x: selectedId ? "-102%" : "0%" }} transition={{ duration: 0.55, ease: EASE }}>
          ...
          <div className="absolute inset-0" style={{ background: "linear-gradient(to top, rgba(0,0,0,0.86), rgba(0,0,0,0.34) 48%, rgba(0,0,0,0.20))" }} />
          <div className="absolute bottom-0 inset-x-0 p-3 text-white" style={{ textShadow: "0 2px 12px rgba(0,0,0,0.7)" }}>
            <p className="text-[9px] uppercase tracking-[0.2em] font-bold opacity-80">Wallets · samen</p>
            <p className="text-[44px] leading-[0.95] font-display font-bold tabular-nums mt-1 tracking-[-0.03em]">{fmtEuro(totalSum)}</p>
            <p className="text-[9px] uppercase tracking-[0.14em] mt-1.5 opacity-65">{active.length} wallets · tik er één</p>
          </div>
        </motion.div>
      </div>
    </div>
