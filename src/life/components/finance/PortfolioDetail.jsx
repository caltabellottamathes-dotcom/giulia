        ...
        <div className="mt-10 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-xl bg-white/55 backdrop-blur-md border border-white/60 p-3">
              ...
              <p className="text-2xl font-display font-semibold tabular-nums" style={{ color: "hsl(var(--d-focus-deep))" }}>{fmtEuro(balance)}</p>
            </div>
            <div className="rounded-xl bg-white/55 backdrop-blur-md border border-white/60 p-3">
              ...
              {points.map((p, i) => {
                ...
                return (
                  <div key={i} className="flex-1 flex flex-col items-center gap-1">
                    <div className="w-full rounded-t" style={{ height: `${h}%`, background: p.balance < 0 ? "hsl(var(--d-focus-urgent))" : "hsl(var(--d-focus-deep))", opacity: 0.55 + (i / points.length) * 0.45 }} />
              ...
              <div className="space-y-1.5">
                {txs.map((t) => (
                  <div key={t.id} className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-display font-semibold tabular-nums" style={{ color: t.type === "expense" ? "hsl(var(--d-focus-deep))" : "hsl(var(--olive))" }}>{t.type === "expense" ? "−" : "+"}{fmtEuro(t.amount)}</span>
                  </div>
                ))}
              </div>
            </div>
