...
import { ArrowLeft, CircleDot, Briefcase, ListChecks, Milestone, Gavel, Lightbulb, FileText, MessageSquare, Phone, Film } from "lucide-react";
import ProjectsStudioCard from "@/focus/components/projects/ProjectsStudioCard";
...
const TABS = [
  ...
  { key: "ACTIVE", label: "Projecten", icon: Briefcase },
  ...
  { key: "INSIGHTS", label: "Inzichten", icon: Lightbulb },
...
export default function ProjectsStudio() {
  ...
  // Project-widget edit-knop → editor (FloatingPanel) op paginaniveau.
  useEffect(() => {
  ...
  // Bij openen van de pagina: de analyse tonen (analyse-stage).
  useEffect(() => {
  ...
  const stageContent =
    ...
    stage === "doc" ? <DocStage /> :
    stage === "analyse" ? <ProjectAnalysisStage tab={tab} onClose={() => setPanelOpen(false)} /> :
    <MediaStage />;

  const tabTitle = TABS.find((t) => t.key === tab)?.label || "Projecten";
    ...
    <div className="fixed inset-x-0 top-14 bottom-0 overflow-visible z-[30]">
      ...
      {!isStage &&
        <motion.div initial={{ opacity: 0, y: -14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: EASE, delay: 0.15 }}
          ...
          <p className="text-[10px] uppercase tracking-[0.28em] text-life-olive font-semibold">FOCUS → PROJECTS</p>
