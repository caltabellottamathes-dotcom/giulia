...
import ProjectPhotoPanel from "@/focus/components/projects/ProjectPhotoPanel";
...
export default function ProjectsStudioDetail() {
  ...
  const loadProject = () => {
    if (!id) return;
    base44.entities.Project.get(id).then(setProject).catch(() => setProject(null));
  };
    ...
    <div className="fixed inset-x-0 top-14 bottom-0 overflow-visible z-[30]">
      ...
      <motion.div initial={{ x: "118%" }} animate={{ x: isStage ? "-24vw" : 0 }} transition={{ duration: 0.7, ease: EASE, delay: first ? 0.15 : 0 }}
        ...
        <div className="relative flex-1 ml-[2.5%] -mt-[78px] -mb-[70px] min-w-0">
          {project && <ProjectPhotoPanel project={project} />}
          <AnimatePresence>
            {isStage &&
