...
const PHOTO = IMAGES.focusLeanPanel;
...
/** WaitingOnYouFocusWidget — G·4:3·R·SIDE · "Waiting on you."
 *  Focus-twin. Foto = focusLeanPanel. Ghost-getal open approvals + 3-woord
 *  samenvattingen. Burgundy/cream. */
export default function WaitingOnYouFocusWidget() {
  const { openModule } = usePanel();
  const { data: approvals, loading } = useEntityList("Approval", { filter: { status: "pending" }, realtime: true });

  const sorted = [...(approvals || [])].sort((a, b) => (b.category === "urgent" ? 1 : 0) - (a.category === "urgent" ? 1 : 0));
  const top3 = sorted.slice(0, 3);
  const total = (approvals || []).length;

  return (
    <div className="w-full h-[340px]">
      <GlassPhotoLayeredWidget shape="4:3" photo={PHOTO} photoPosition="right" photoFraction={0.42} overhang={0} domain="focus" radius="large" onClick={() => openModule("approvals")} photoOverlay="bg-gradient-to-t from-black/45 via-black/12 to-transparent">
        <div className="pointer-events-none absolute" style={{ left: "44%", bottom: "0%", transform: "translateX(-50%)" }}>
          <span style={{ color: "hsl(var(--d-focus-deep))", opacity: 0.32 }}>
        ...
        <div className="flex flex-col gap-2">
          {loading ? (
            ...
            top3.map((a, i) => {
              ...
              return (
                <motion.button key={a.id} onClick={(e) => { e.stopPropagation(); openModule("approvals"); }} initial={{ opacity: 0, x: 8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.08, duration: 0.4 }} className="group flex items-center gap-3 w-full text-left rounded-xl px-2 py-2 hover:bg-white/5 transition-colors">
                  ...
                  <span className="flex-1 min-w-0">
                    <span className="block text-[15px] font-display font-bold uppercase tracking-[0.01em] leading-tight truncate" style={{ color: IVORY }}>{words.join(" ")}</span>
                  </span>
