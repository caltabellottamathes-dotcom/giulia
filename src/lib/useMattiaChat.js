 ...
 * Gemini-key — géén integration-credits). Persona/instructies via
 * mattia.jsonc (mattiaInstructions.ts) + Playtime-context, niet via
 * mattiaPrompt.ts. Parallel aan useGiuliaChat, voor de Mattia-draad
 * (Message role="mattia").
 *
 * chatWithMattia is een multi-step Gemini-loop. We abonneren ons op de
 * Message-entiteit; zodra Mattia zijn antwoord opslaat (role="mattia"),
 * verschijnt het direct in het chatvenster.
 */
const norm = (m) => ({
  id: m.id,
...
export function useMattiaChat() {
  ...
  const send = useCallback(async (content, opts = {}) => {
    ...
    try {
      ...
      // Media-commando's (camera/mediatheek) ALTIJD afvuren — ook als het
      // antwoord al via realtime binnenkwam, anders opent de camera nooit.
