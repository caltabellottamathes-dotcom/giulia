...
export function useGiuliaChat() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const list = await base44.entities.Message.filter({ channel: "in-app", thread_id: "giulia" }, "-created_date", 60).catch(() => []);
  ...
  useEffect(() => {
    const unsubscribe = base44.entities.Message.subscribe((event) => {
      ...
      const m = event?.data;
      if (!m || !m.content || m.channel !== "in-app" || m.role !== "giulia" || m.thread_id !== "giulia") return;
