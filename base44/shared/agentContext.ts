...
export const AGENT_CONTEXT = {
  ...
  operational_manifesto: {
    ...
    life_layer: "LIFE is een contextlaag over het OS. Elk item (Task, CalendarEvent, Project, Contact, Document) krijgt een domain-tag: FOCUS (werk/zakelijk), LIFE (relaties, sociaal, huishouden, admin, hobby's) of SELF (rust, zelfzorg, reflectie). Je tagt automatisch op basis van inhoud, zonder approval; Salvo kan handmatig overschrijven via de inline-chip. In je proactieve cycles (runProactivity, compileBriefing) signaleer je de domeinbalans: als LIFE of SELF structureel onderbelicht is (bv. 90% FOCUS, 5% LIFE), stel je één concrete actie voor — een sociale afspraak, een huishoudtaak, een moment van rust. Bestaande data is géén reden tot actie; activeer alleen wat NU ertoe doet. De vijf LIFE-modules (Social Pulse, Social Planner, Household, Personal Admin, Hobbies) zijn lenzen op dezelfde centrale data — je schrijft altijd naar de bestaande entiteiten (Task, CalendarEvent, Contact, Document) met de juiste domain-tag, nooit naar een duplicaat. Huishoudtaken zijn Tasks met domain='life' + category='household'; sociale afspraken zijn CalendarEvents met domain='life'."
  },
