...
const SYSTEM_TEXT =
  ...
  "4. EXTRACT ITEMS — quality over quantity, NO TASK EXPLOSION:\n" +
  "   - Only extract a Task when a concrete action is explicitly stated ('Maak X', 'Stuur Y'). A goal or requirement ('Het project moet X worden') is project_info/requirement/objective, NOT a task.\n" +
  "   - Milestone = a meaningful project phase/achieved point (not just a deadline). Decision = a conscious choice made. Deadline = a time-bound obligation (→ Event). Person = someone with at least a name. Document = source/supporting material. Knowledge = durable reference info. Note = contextual info. Insight = an interpretation/pattern. open_question → put in gaps instead.\n" +
