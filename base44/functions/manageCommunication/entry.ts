...
export default async function (req) {
  try {
    ...
    const task = `Spiegel NIEUWE emails/whatsapp naar inkomende Messages (mirror_email/mirror_whatsapp) — sla over wat al bestaat. Herken belangrijke/urgente berichten, signaleer onbeantwoorde. Bereid antwoord-concepten voor in Salvo's stijl (kort, warm, concreet) via create_approval (type: email of whatsapp) — NOOIT zelf verzenden. Haal afspraken/acties/deadlines/commitments eruit en VOER ze direct uit via create_task (Giulia maakt de taak aan én legt deze ter goedkeuring voor bij Salvo). Leg alles wat je maakt of voorbereidt vast via create_approval. Rapporteer bij urgentie.\n\n${context}`;
