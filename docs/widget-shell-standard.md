...
Een widget **behoudt altijd zijn oorspronkelijke ratio**. Bij een "due"/groot
moment groeit de widget uitsluitend van **3 span → 6 span** (breder én hoger
in dezelfde ratio). **Verander nooit de ratio, en schaal andere widgets of
tekst niet mee** — schakel daarvoor de masonry `fitHeight` uit (zie Home.jsx).
